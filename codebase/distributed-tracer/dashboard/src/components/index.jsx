...
const XPAD = 60;
const YPAD = 20;
const VERT_SPACE = 130;
const VERT_PAD = 60;
const CLASS_WIDTH = 120;
...
export default function (props) {
    ...
    useEffect( _ => {
         console.log("inside use effects")
          axios.get(`http://localhost:8081/services`)
            .then(res => {
                const serviceNodes = res.data.map(node=>node.label);
                console.log("axios")
                console.log(serviceNodes)
                setLabels(serviceNodes);
            })
    ...
    return (
        <div className={classes.root}>
            ...
            <svg ref={ref} height={"100%"} width={"80%"} className={classes.svg}/>
